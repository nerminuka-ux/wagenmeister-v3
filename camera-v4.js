(()=>{
  ...
  function dbNumbers(){
    const out=new Set();
    try{if(typeof db!=='undefined'&&Array.isArray(db))db.forEach(w=>{const d=norm(w?.number);if(d.length===12)out.add(d);});}catch{}
    try{const saved=JSON.parse(localStorage.getItem('wm4s_db')||'[]');saved.forEach(w=>{const d=norm(w?.number);if(d.length===12)out.add(d);});}catch{}
    return out;
  }
  ...
  function renderVariant(src,sx,sy,sw,sh,targetW,mode){
    ...
    for(let i=0;i<d.length;i+=4){
      const g=.299*d[i]+.587*d[i+1]+.114*d[i+2];let v;
      if(mode==='hard')v=g>170?255:0;else if(mode==='invert')v=g>170?0:255;else v=Math.max(0,Math.min(255,(g-95)*2.2));
      d[i]=d[i+1]=d[i+2]=v;
    }
  ...
  async function makeVariants(file){
    ...
    const crops=[[0,0,w,h],[Math.round(w*.12),Math.round(h*.24),Math.round(w*.76),Math.round(h*.42)],[Math.round(w*.18),Math.round(h*.30),Math.round(w*.64),Math.round(h*.30)],[Math.round(w*.22),Math.round(h*.34),Math.round(w*.58),Math.round(h*.22)]];
    for(const [sx,sy,sw,sh] of crops){const tw=Math.min(2800,Math.max(1500,Math.round(sw*2.4)));v.push(renderVariant(bmp,sx,sy,sw,sh,tw,'contrast'));v.push(renderVariant(bmp,sx,sy,sw,sh,tw,'hard'));v.push(renderVariant(bmp,sx,sy,sw,sh,tw,'invert'));}
    return v;
  }
  ...
  input.addEventListener('change',async e=>{
    const file=e.target.files?.[0];if(!file)return;say('Foto aufgenommen · Wagennummer wird erkannt …');
    try{
      ...
      const variants=await makeVariants(file),known=dbNumbers(),counts=new Map();
      for(let i=0;i<variants.length;i++){
        ...
        for(const c of candidatesFrom(r?.data?.text||''))counts.set(c,(counts.get(c)||0)+1);
      }
      const ranked=[...counts].map(([n,c])=>({n,c,valid:validUIC(n),known:known.has(n)})).sort((a,b)=>(b.known-a.known)||(b.valid-a.valid)||(b.c-a.c));
      const bestKnown=ranked.find(x=>x.known&&x.valid);
      if(bestKnown){fill(bestKnown.n);say('✓ Erkannt und in Datenbank gefunden: '+fmt(bestKnown.n),true);return;}
      const bestValid=ranked.find(x=>x.valid);
      if(bestValid){fill(bestValid.n);say('Nummer erkannt, aber nicht in der Wagen-Datenbank. Bitte vor Übernahme kontrollieren: '+fmt(bestValid.n));return;}
      say('Keine sichere Wagennummer erkannt. Bitte näher fotografieren und nur die Wagennummer ins Bild nehmen.');
